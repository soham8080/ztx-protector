...
async def sell_character(update: Update, context: CallbackContext) -> None:
    try:
        # Parse the character ID from the command arguments
        args = context.args
        if len(args) != 1:
            await update.message.reply_text("Invalid format. Use: /sell <character_id>")
            return

        character_id = args[0]

        # Retrieve user data including wallet balance and character collection
        ...
        # Find the character to sell in the user's collection
        sold_character = None
        for character in user.get("characters", []):
            if character["id"] == character_id:
                sold_character = character
                break

        if not sold_character:
            await update.message.reply_text("Character not found in your collection.")
            return

        # Define the sell prices based on rarity
        rarity_sell_prices = {
            1: 30,   # Common
            2: 40,   # Rare
            3: 80,   # Legendary
            4: 50,   # Medium
            5: 125,   # Special Edition
            6: 150,    # Limited Edition
            7: 240, # Pemium Edition 
        }

        rarity = sold_character.get("rarity", 1)  # Default to Common if rarity is not specified
        sell_price = rarity_sell_prices.get(rarity, 10)  # Default to a minimum sell price if rarity is not found

        # Deduct coins from user's wallet
        current_balance = user.get("coins", 0)
        new_balance = current_balance + sell_price

        # Remove the sold character from the user's collection
        user["characters"].remove(sold_character)

        # Update user data in the database
        await user_collection.update_one(
            {"id": user_id},
            {"$set": {"coins": new_balance, "characters": user["characters"]}}
        )

        await update.message.reply_text(f"Successfully sold {sold_character['name']} for {sell_price} coins. New balance: {new_balance}")

    except Exception as e:
        LOGGER.error(f"Error occurred: {e}")
        await update.message.reply_text("An error occurred while selling the character. Please try again later.")
...
async def sell_character(update: Update, context: CallbackContext) -> None:
    try:
        ...
        rarity_sell_prices = {
            ...
            3: 30,   # Legendary
            4: 40,   # Medium
            5: 50,   # Special Edition
            6: 60    # Limited Edition
        }
...
def get_rarity_name(rarity: int) -> str:
    ...
    return rarity_map.get(rarity, "Unknown Rarity")
...
SELL_CHARACTER_HANDLER = CommandHandler('sell', sell_character)


application.add_handler(SELL_CHARACTER_HANDLER)
